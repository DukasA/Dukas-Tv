...
export const NavLink: React.FC<ILinkProps> = ({
  ...
  if (type === 'noDropDown') {
    ...
    return (
      <li
        className="flex whitespace-nowrap text-white sm:text-sm md:text-xl lg:text-2xl mr-2 lg:mr-8 border-[1px] p-2 pl-4 pr-4 hover:cursor-pointer hover:bg-[#1F80E0] ease-out duration-300 rounded-3xl"
        onClick={onClick}
      >
        <Link to={link}>{title}</Link>
      </li>
